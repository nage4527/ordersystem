...
function doPost(e) {
  try {
    ...
    console.log('e 物件存在:', !!e);
    console.log('e 物件內容:', e);
    
    // 安全檢查 e 是否存在
    if (!e) {
      throw new Error('未收到請求參數');
    }
    
    console.log('e.parameter 存在:', !!e.parameter);
    console.log('e.parameters 存在:', !!e.parameters);
    console.log('e.postData 存在:', !!e.postData);
    ...
    console.log('取餐地點檢查:', data.pickupLocation);
    console.log('取餐地點類型:', typeof data.pickupLocation);
    console.log('取餐地點是否為空:', !data.pickupLocation);
    
    // 詳細檢查每個必要欄位
    console.log('customerName:', data.customerName, '是否存在:', !!data.customerName);
    console.log('customerPhone:', data.customerPhone, '是否存在:', !!data.customerPhone);
    console.log('pickupDate:', data.pickupDate, '是否存在:', !!data.pickupDate);
    console.log('pickupTime:', data.pickupTime, '是否存在:', !!data.pickupTime);
    console.log('pickupLocation:', data.pickupLocation, '是否存在:', !!data.pickupLocation);
    
    // 驗證必要欄位
    if (!data.customerName || !data.customerPhone || !data.pickupDate || !data.pickupTime || !data.pickupLocation) {
      const missingFields = [];
      if (!data.customerName) missingFields.push('customerName');
      if (!data.customerPhone) missingFields.push('customerPhone');
      if (!data.pickupDate) missingFields.push('pickupDate');
      if (!data.pickupTime) missingFields.push('pickupTime');
      if (!data.pickupLocation) missingFields.push('pickupLocation');
      
      throw new Error('缺少必要的訂單資訊: ' + missingFields.join(', '));
    }
...
function setupSheetHeaders(sheet) {
  ...
  sheet.setColumnWidth(7, 120); // 取餐地點
  sheet.setColumnWidth(8, 350); // 訂購品項
  sheet.setColumnWidth(9, 80);  // 總金額
  sheet.setColumnWidth(10, 200); // 備註
  sheet.setColumnWidth(11, 80); // 狀態
  ...
  // 設定訂購品項欄位（H欄）預設為自動換行
  const itemsColumn = sheet.getRange('H:H');
  itemsColumn.setWrap(true);
  itemsColumn.setVerticalAlignment('top');
  itemsColumn.setHorizontalAlignment('left');
  
  // 設定 H 欄的最小寬度以確保自動換行效果
  sheet.setColumnWidth(8, 350); // 增加 H 欄寬度到 350 像素
...
function formatNewRow(sheet, rowNumber) {
  const range = sheet.getRange(rowNumber, 1, 1, 11);
  
  // 設定邊框
  range.setBorder(true, true, true, true, true, true);
  
  // 設定對齊方式
  sheet.getRange(rowNumber, 1, 1, 7).setHorizontalAlignment('center'); // 前7欄置中
  sheet.getRange(rowNumber, 9).setHorizontalAlignment('right'); // 金額右對齊
  
  // 設定訂購品項欄位（H欄）自動換行和垂直對齊
  const itemsCell = sheet.getRange(rowNumber, 8); // 第8欄是訂購品項
  itemsCell.setWrap(true); // 啟用自動換行
  itemsCell.setVerticalAlignment('top'); // 垂直對齊到頂端
  itemsCell.setHorizontalAlignment('left'); // 水平對齊到左邊
  
  // 設定該行的最小高度以容納多行內容
  sheet.setRowHeight(rowNumber, 80); // 增加行高到 80 像素
...
function initializeSpreadsheet() {
  try {
    ...
    const settingData = [
      ...
      ['', ''],
      ['搭配優惠', ''],
      ['杏仁茶750ml + 杏仁豆腐', '2項=$270 / 3項=$400 / 以此類推']
    ];
...
/**
 * 修復現有 Google Sheets 中的自動換行格式
 */
function fixAutoWrapForAllSheets() {
  try {
    const spreadsheet = SpreadsheetApp.openById(SPREADSHEET_ID);
    console.log('開始修復所有工作表的自動換行格式');
    ...
    sheets.forEach(sheet => {
      ...
      // 跳過設定工作表
      if (sheetName === '設定') {
        console.log(`跳過設定工作表: ${sheetName}`);
        return;
      }
      
      console.log(`正在修復工作表: ${sheetName}`);
      ...
      if (lastRow > 1) { // 跳過標題行
        for (let i = 2; i <= lastRow; i++) {
          const itemsCell = sheet.getRange(i, 8); // H欄是訂購品項
          itemsCell.setWrap(true);
          itemsCell.setVerticalAlignment('top');
          itemsCell.setHorizontalAlignment('left');
      ...
      console.log(`✅ 完成修復工作表: ${sheetName}，共處理 ${lastRow - 1} 行資料`);
    });
    
    // 強制刷新格式
    SpreadsheetApp.flush();
    
    console.log('✅ 所有工作表的自動換行格式修復完成');
    return '✅ 所有工作表的自動換行格式修復完成';
    
  } catch (error) {
    console.error('修復自動換行格式失敗:', error);
 ...
 * 用於調試取餐地點無法寫入的問題
 */
function debugPickupLocationIssue() {
  console.log('=== 開始調試取餐地點問題 ===');
  
  // 模擬前端發送的資料格式
  const mockWebFormData = {
    customerName: "網頁測試客戶",
  ...
  console.log('模擬網頁表單資料:', JSON.stringify(mockWebFormData, null, 2));
  
  // 檢查資料完整性
  console.log('=== 資料完整性檢查 ===');
  console.log('customerName 存在:', !!mockWebFormData.customerName);
  console.log('customerPhone 存在:', !!mockWebFormData.customerPhone);
  console.log('pickupDate 存在:', !!mockWebFormData.pickupDate);
  console.log('pickupTime 存在:', !!mockWebFormData.pickupTime);
  console.log('pickupLocation 存在:', !!mockWebFormData.pickupLocation);
  console.log('pickupLocation 值:', mockWebFormData.pickupLocation);
  console.log('pickupLocation 類型:', typeof mockWebFormData.pickupLocation);
  
  try {
    // 嘗試儲存到 Google Sheets
    const result = saveOrderToSheets(mockWebFormData);
    console.log('✅ 調試測試成功:', result);
    return `✅ 調試測試成功！訂單編號: ${result.orderNumber}，工作表: ${result.sheetName}`;
  } catch (error) {
    console.error('❌ 調試測試失敗:', error);
    console.error('❌ 錯誤堆疊:', error.stack);
    return `❌ 調試測試失敗: ${error.message}`;
  }
}